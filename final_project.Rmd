...
renderPlot({
  ...
  ggplot(filtered_songs, aes(y = .data[[input$y]], x = album_release_year, color = genre)) + 
    ...
    labs(x = "Album Release Year", color = "Genre", title = "Audio Features Over Time") + 
    theme_classic() + 
...
renderPlot({
  ...
  ggplot(filtered_songs, aes(x = .data[[input$y]], fill = genre)) + 
    geom_density(alpha = 0.5) + 
    labs(fill = "Genre", title = "Comparing Audio Features for Two Genres") +
    theme_classic() + 
...
renderPlot({
  ggplot(genre3(), aes(x = popularity, y = .data[[input$y5]], color = genre)) +
    ...
    labs(color = "Genre", x = "Popularity", title = "The Relationship Between Artist Popularity and an Audio Feature for Selected Genres") + 
    theme_classic() + 
    scale_color_calc()
})
```
...
- The top artists for R&B, EDM, and Pop have the most songs per album in 2020.
...
- In general, major seems to be more frequently used by top artists than minor. (Note: In the next section we will examine if this is skewed by specific artists) 

- Top EDM, Latin, and Hip-Hop artists do not show much of a preference for a specific mode.

- Country and Christian songs produced by the top artists are almost always in major.

### **Observations / Conclusions Of Right Plot:**

- The A key is most common in Rock songs.

- The B and E keys are used very similarly across all genres. 

- The C key is most common in Christian and Pop songs. It is, however, one of the most commonly used keys in almost all genres.

- The C# key is used by far the most in Rap and Hip-Hop. In addition to those genres, it is also the most commonly used key in Latin. 

- The D key is most commonly used in Christian and Rock songs. 

- The D# key is rarely used across all genres. 
...
- **R&B:** 
  ...
  - All the top R&B artists use/used the A# key at a similar rate. 
...
- **Christian:**
  ...
  - Hillsong Worship rarely uses keys with sharps (#).


- **Country:**

  - Zach Bryan uses key F# way less than the other top Country artists.
  - Zach Bryan uses a lot of key C, especially compared to the other top country artists.
  - Luke Combs rarely uses key A. 
...
- **Indie:** 
  ...
  - Hozier uses the B key way less than the other two popular Indie artists. In fact, the B key is one of Hozier's  least used keys while it is the Arctic Monkeys and  Noah Kahan's most used key. 
...
- **Latin:** 
  ...
  - All three use the D# key the least. 
  - In general the distribution of keys used is almost the same for all three of the top Latin artists. 
...
- **Pop:** 
  ...
  - Taylor Swift does not commonly use keys with sharps (#).
...
- **Rock:** 
  ...
  - Imagine Dragons use the C key the most. 
  - Linkin Park uses the C# key the most. 
  - Queen used the D key the most. 
  - The Beattles used A key, C key, D key, E key, and G key frequently (almost equally). They rarely used any keys with sharps (#). 
