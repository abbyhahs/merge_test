...
Row
-------------------------
The viewer has the ability to compare different genres and observe how the audio features have changed over time or compare the frequency of various audio features.
...
Our second question was how do the various audio features differ for each genre based on each of the top artist's popularity score out of 100.
...
renderPlot({
  ggplot(genre3(), aes(x = popularity, y = .data[[input$y5]], color = genre)) +
    ...
    labs(color = "Genre", x = "Popularity") + theme_classic() + scale_color_calc()
})
```
...
The ability to select any number of the 10 genres gives viewers the freedom to explore differences in audio features based on  each artists popularity score.
...
Viewers have the ability to change the genre that is shown in order to find the most popular artists of each genre. They can then observe how each artist's number of songs per genre has changed over time and how the artist's compare to each other in regards to number of songs per album.
