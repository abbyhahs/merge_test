...
renderPlot({
  ...
  ggplot(aes(y = .data[[input$album3]], x = album_release_year, color = name)) + 
    geom_point() + geom_smooth(method = "lm", se = FALSE) + labs(x = "Album Release Year", color = "Artist Name")+ theme_classic() + scale_color_calc()
})
...
renderPlot({
     if (input$genre_check) 
        ggplot(songs, aes(x = .data[[input$x5]], y = .data[[input$y5]]))  + 
         geom_violin() 
     else if (!input$genre_check)      
        ggplot(songs, aes(x = .data[[input$x5]], y = .data[[input$y5]]))  + 
          geom_violin()
})
